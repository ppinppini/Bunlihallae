...
const Game = (props) => {
  ...
  const [buttonSrc, setButtonSrc] = useState(
    "/resources/pages/main/startButton.png"
  );
  ...
  const [items, setitems] = useState([
    {
      name: "파스퇴르",
      current: [0, 0], //현재좌표
      loc1: [351, 42], //좌측 상단 모서리 좌표
      loc2: [418, 228], //우측 하단 모서리 좌표
      answer: 0,
      success: false,
    },
    {
      name: "아이시스",
      current: [0, 0], //현재좌표
      loc1: [455, 85], //좌측 상단 모서리 좌표
      loc2: [503, 234], //우측 하단 모서리 좌표
      answer: 0,
      success: false,
    },
    {
      name: "떡볶이",
      current: [0, 0], //현재좌표
      loc1: [540, 113], //좌측 상단 모서리 좌표
      loc2: [691, 228], //우측 하단 모서리 좌표
      answer: 1,
      success: false,
    },
    {
      name: "자일리톨",
      current: [0, 0], //현재좌표
      loc1: [724, 25], //좌측 상단 모서리 좌표
      loc2: [802, 133], //우측 하단 모서리 좌표
      answer: 0,
      success: false,
    },
    {
      name: "자껍",
      current: [0, 0], //현재좌표
      loc1: [826, 47], //좌측 상단 모서리 좌표
      loc2: [934, 128], //우측 하단 모서리 좌표
      answer: 0,
      success: false,
    },
    {
      name: "알약",
      current: [0, 0], //현재좌표
      loc1: [784, 167], //좌측 상단 모서리 좌표
      loc2: [921, 254], //우측 하단 모서리 좌표
      answer: 1,
      success: false,
    },
    {
      name: "좋은 물",
      current: [0, 0], //현재좌표
      loc1: [971, 65], //좌측 상단 모서리 좌표
      loc2: [1051, 233], //우측 하단 모서리 좌표
      answer: 1,
      success: false,
    },
  ]);
    ...
    canvas.onmousedown = (e) => {
      ...
      for (let i = 0; i < 7; i++) {
        if (!items_tmp[i].success) {
          //성공된건 생략
          if (
            e.layerX > items_tmp[i].loc1[0] &&
            e.layerX < items_tmp[i].loc2[0] &&
            e.layerY > items_tmp[i].loc1[1] &&
            e.layerY < items_tmp[i].loc2[1]
          ) {
            _selected = i;
          }
        }
    ...
    canvas.onmousemove = (e) => {
      ...
      if (selected != -1) {
        items_tmp[_selected].current[0] =
          _selectedIloc[0][0] + (e.layerX - _selectedloc[0]);
        items_tmp[_selected].current[1] =
          _selectedIloc[0][1] + (e.layerY - _selectedloc[1]);

        items_tmp[_selected].loc1[0] =
          _selectedIloc[1][0] + (e.layerX - _selectedloc[0]);
        items_tmp[_selected].loc1[1] =
          _selectedIloc[1][1] + (e.layerY - _selectedloc[1]);

        items_tmp[_selected].loc2[0] =
          _selectedIloc[2][0] + (e.layerX - _selectedloc[0]);
        items_tmp[_selected].loc2[1] =
          _selectedIloc[2][1] + (e.layerY - _selectedloc[1]);
      }
      setitems(items_tmp);
    };
  };
  ...
  const [isGameResultPosted, setIsGameResultPosted] = useState(false);
  const successPage = (ctx) => {
    ...
    if (!isGameResultPosted) {
      setIsGameResultPosted(true); // 상태를 먼저 true로 설정
      postGameResultApi(100, token)
          .then(response => {
            console.log('Game result posted successfully:', response);
            // 10초 후에 홈 화면으로 이동
            setTimeout(() => {
              navigate("/home");
            }, 2000);
          ...
          .catch(error => {
            ...
            setIsGameResultPosted(false); // 에러 발생 시 상태를 다시 false로 설정
          });
    }
  };
